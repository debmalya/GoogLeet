...
public class LexigraphicallyLargest {
   ...
   *
   * <p>The integer 1 occurs once in the sequence. Each integer between 2 and n occurs twice in the
   * sequence. For every integer i between 2 and n, the distance between the two occurrences of i is
   * exactly i. The distance between two numbers on the sequence, a[i] and a[j], is the absolute
   * difference of their indices, |j - i|.
   *
   * <p>Return the lexicographically largest sequence. It is guaranteed that under the given
   * constraints, there is always a solution.
   *
   * <p>A sequence a is lexicographically larger than a sequence b (of the same length) if in the
    ...
    if (ans[index] != 0)
      ...
      for (int i = n; i >= 1; i--) {
        ...
        if (i == 1) {
          ...
          ans[i + index] =
              i; // assigning the second occurence of i in the desired position i.e, (current index
                 // + i )
