...
public class ShortestPathWithAlternateColor {
	 ...
	 * ai to node bi in the graph, and blueEdges[j] = [uj, vj] indicates that there
	 * is a directed blue edge from node uj to node vj in the graph. Return an array
	 * answer of length n, where each answer[x] is the length of the shortest path
	 * from node 0 to node x such that the edge colors alternate along the path, or
	 * -1 if such a path does not exist.
	 * 
	 * Example 1: Input: n = 3, redEdges = [[0,1],[1,2]], blueEdges = [] Output:
	 * [0,1,-1]
	 * 
	 * 
	 * Example 2:Input: n = 3, redEdges = [[0,1]], blueEdges = [[2,1]] Output:
	 * [0,1,-1]
	 * 
	 * 
	 * @param n
